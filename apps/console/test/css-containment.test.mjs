...
test('quoted braces and commas followed by a leak', () => {
...
test('escaped comma does not split, but real comma after does expose leak', () => {
  // An escaped comma is NOT a selector separator — so this is one rooted selector, valid
  assert.equal(validateLegacyCompatibilityCss(`${R} .a\\, .b { color: red; }`), 1);
  // But a real comma after an escaped-comma selector can expose a leak
...
// ─── Nested rule ───
test('rejects illegal nested rule inside declaration block', () => {
  assert.throws(() => validateLegacyCompatibilityCss(`${R} .a { ${R} .b { color: red; } }`), /nested \{/i);
});

// ─── Selector delimiter balance ───
...
test('rejects mismatched delimiters in selector', () => {
  assert.throws(() => validateLegacyCompatibilityCss(`${R} :is([) { color: red; }`), /mismatched|unclosed/i);
});

// ─── Declaration delimiter balance ───
test('rejects mismatched ) in declaration', () => {
...
test('rejects mismatched ) in @supports prelude', () => {
